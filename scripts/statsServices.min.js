...
function cargareventotable(events) {
  ...
  let mayorpocentaje = (
    (mayorasis.assistance / mayorasis.capacity) *
    100
  ).toFixed(2);
  let menorporcentaje = (
    (menorasis.assistance / menorasis.capacity) *
    100
  ).toFixed(2);
...
function cargarupcomingtable(upcomingeventsp) {
  ...
  let categoriassumas = array_upcomingevents.reduce((anterior, actual) => {
    const categoriasencontradas = anterior.find(
      (ant) => ant.category == actual.category
    );
...
function cargapasteventstable(pasteventsp) {
  ...
  // sumamnos el estimado de cada categoria en total
  let categoriassumas = array_pastevents.reduce((anterior, actual) => {
    const categoriasencontradas = anterior.find(
      (ant) => ant.category == actual.category
    );
    if (categoriasencontradas) {
